...
export default async function AdminDashboardPage() {
  ...
  const gmv = deliveredOrders?.reduce((sum, o) => sum + Number(o.ord_total_amount ?? 0), 0) ?? 0
  // Platform revenue is the fee cut, not the full order value — most of
  // that total goes to the chef, not the platform. See lib/platformFee.ts.
  ...
  const stats = [
    ...
    { label: 'Order value (GMV)', value: formatCurrency(gmv) },
